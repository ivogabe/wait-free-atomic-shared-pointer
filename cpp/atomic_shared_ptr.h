...
struct control_block {
  ...
  public:
    ...
    inline void rc_decrement(uint64_t count) {
      uint64_t rc = this->reference_count.fetch_sub(count, std::memory_order::memory_order_release);
      if (rc == count) {
        // Load as an acquire memory barrier
        this->reference_count.load(std::memory_order::memory_order_acquire);
        delete this->ptr;
        delete this;
      } else if (rc < count) {
...
inline std::memory_order memory_order_load(std::memory_order order) {
  if (order == std::memory_order::memory_order_release) {
    return std::memory_order::memory_order_relaxed;
  }
  if (order == std::memory_order::memory_order_acq_rel) {
...
struct atomic_shared_ptr {
  private:
    ...
    void shift_references(::control_block<T> *control_block, uint64_t tag) {
      ...
      while (true) {
        ...
        if (this->tagged_pointer.compare_exchange_strong(expected, new_value, std::memory_order::memory_order_relaxed, std::memory_order::memory_order_relaxed)) {
          // Success!
          return;
        }
  ...
  public:
    ...
    ~atomic_shared_ptr() {
      size_t tagged_pointer = this->tagged_pointer.load(std::memory_order::memory_order_relaxed);
